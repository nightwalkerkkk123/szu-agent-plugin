...
class ErrorCodeTest {
    ...
    @Test
    @DisplayName("has exactly 53 constants")
    void hasThirtyEightConstants() {
        // 14 original + 1 RATE_LIMITED + 3 SESSION_* + 3 ATTACHMENT_* + 5 SCHEDULE_*
        ...
        // + 3 EXTERNAL_SKILL_* + 8 PAYMENT_* = 53
        assertThat(ErrorCode.values()).hasSize(53);
    }

    @Test
    @DisplayName("RATE_LIMITED 元数据正确")
    void rateLimitedMetadata() {
        ErrorCode c = ErrorCode.RATE_LIMITED;
        ...
        assertThat(c.hint()).isEqualTo("请求过于频繁，请稍后再试");
    }
