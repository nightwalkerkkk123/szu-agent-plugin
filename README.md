...
深圳大学内部网存在大量重复、固定、流程化的操作（体育场馆预约、公文通查询、畅课任务、成长方案、邮件草稿等）。本项目将这些操作封装成**标准化的 CLI 工具**与**Skill / MCP 插件**，使 OpenClaw、ChatGPT Agent、其他 Agent 系统可以通过命令调用，由本地浏览器（CloakBrowser / Playwright）在用户授权下完成实际网页操作。
